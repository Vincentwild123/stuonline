...
export function createPost(formData, files) {
	return axios.post(api.card.createCard,{formData,files},{
...
export function getModulePostList(postModule, postId, limit) {
	return axios.get(api.card.getModulePostList + '/' + postModule + '/' + postId + '/' + limit)
}
//点赞
...
export function getPostComment(postId, comId, limit) {
	return axios.get(api.card.commonCardFirst + '/' + postId + '/' + comId + '/' + limit)
}
//获取帖子的二级评论
export function getPostSecondComment(postId, comSecond, comId, limit) {
	return axios.get(api.card.commonCardSecond + '/' + postId + '/' + comSecond + '/' + comId + '/' + limit)
}
//创建一级评论--创建二级评论
export function createComment(postId, data) {
	return axios.post(api.card.commonIt + '/'
		+ postId, data,
...
export function getMissionList(lastMissionId, pageSize) {
	return axios.get(api.mission.getMissonList + "/" + lastMissionId + '/' + pageSize)
}
//查询最后的任务的id

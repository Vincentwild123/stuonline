<template>
	<view class="contain flex-column" v-if="isLoad">
		<!-- 包括返回图标的页面顶部，固定不动的部分 -->
		<pageTop :managedTitleIndex="managedTitleIndex" :isLeader="isLeader" title="团队成员" :backTitle="teamInfo.teamName" class="top" eventName="showTeamMembersBack"></pageTop>
		<!-- 顶部结束 -->
		
		<!-- 团队成员的滚动条 -->
		<view class="scroll" >
			<scroll-view scroll-y class="list-scroll flex-column" :class="isManagedState">
				<!-- 团队的队长 -->
				<view class="biaoti letallCenter"><text>队长</text></view>
				<member-item class="" :isManaged="isManaged" :isLeader="true" :isShow="true" :teamMemberInfo="teamInfo.leaderInfo"></member-item>
				<!-- 队长结束 -->
				<!-- 团队的普通成员，多个，使用for -->
				<view class="biaoti letallCenter"><text>队员</text></view>
				<memberItem v-for="(item,index) in teamMembers" class="member-item" 
							:isManaged="isManaged" :index="index" :isLeader="false" :teamMemberInfo="item"></memberItem>
				<!-- 普通成员结束 -->
			</scroll-view>
		</view>
		<!-- 滚动条结束 -->

<template>
	<view class="item-members flex-row " style="">
		<view class="item-members-left">
			<view class="item-members-left-title letColumnCenter"><text>团队成员({{teamMembers.length+1}})</text></view>
			<scroll-view scroll-x class="scroll-list" show-scrollbar="true">
				<view class="letColumnCenter">
					...
					<view class="flex-column scroll-item1" @click="addTeamMembers">
						<image src="../../../../UI/team/createTeam-img.png" class="img"></image>
					...
					<view class="flex-column scroll-item2">
						...
						<view class="letRowCenter member-name"><text>{{teamLeaderInfo.userName}}</text></view>
					</view>
					<!-- 一般成员 -->
					<view class="flex-column scroll-item1" v-for="(item,index) in teamMembers">
						<image :src="item.userHead" class="img"></image>
						<view class="letRowCenter member-name"><text>{{item.userName}}</text></view>
					</view>
				</view>
			</scroll-view>
		</view>
		<view class="item-members-right letColumnCenter">
			<uniIcon type="jiantou" size="15" color="#ADB7D1" eventName="teamDetail-join" index="3"></uniIcon>
		</view>
	</view>
</template>

<template>
	<view class="contain" v-if="!isLoad">
		<pageTop class="Compos-top" eventName="teamDetail-joinback" backTitle="我的团队" :title="teamInfo.teamName">
		</pageTop>
		<!-- 团队详情区 -->
		...
		<view class="compos-teamMembers">
			<team-member class="compos-teamMembers-item" :teamLeaderInfo="teamInfo.leaderInfo"
				:teamMembers="teamInfo.membersInfo"></team-member>
		</view>
		<!-- 3个选择操作区 -->
		<view class="letColumnCenter item-xuanze" v-for="(item, index) in xuanze1" v-if="item.isShow"
			@click="navigateToOtherPage(index)">
		...
		<!-- 解散团队或退出团队 -->
		<view class="letallCenter item-xuanze"><text class="item-xuanze-text1">{{ xuanze2[xuanze2_index] }}</text>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			isLeaderJudge() {
				...
				if (this.teamInfo.leaderInfo.userId === userId) {
					//判断和团队队长的userId是否相同
					this.isLeader = true;
				} else {
					this.isLeader = false;

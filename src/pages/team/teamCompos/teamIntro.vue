<template>
  ...
  <view class="contain flex-column" @click="teamIsClick">
    <view class="top flex-row">
      <view class="top-left flex-row">
        ...
        <view class="teamImg-right flex-column">
          <!-- 团队名字 -->
          <view class="teamName letColumnCenter"
            >{{ teaminfor.teamName }}({{ teamMembersLength }})</view
          >
          <!-- 团队队徽和队长 -->
    ...
    <!-- 团队的介绍 -->
    <view class="bottom" :class="showRow"
      ><text>{{ teaminfor.teamIntro }}</text></view
    >
  </view>
  <!-- 一个团队介绍完毕 -->
</template>
...
<script>
import { getTeamInfoById } from "../service.js";
    ...
    teamIsClick(id) {
      if (!this.isNoJoin) {
        if (this.myTeam) {
          uni.navigateTo({
            url:
              "./subPages/teamDetail-join?teamId=" +
              this.teaminfor.teamId,
          });
        } else {
          uni.navigateTo({
            url:
              "./subPages/teamDetail-nojoin?teamId=" +
              this.teaminfor.teamId,
          });
        }
      }
